 ...
 *
 * Functions that output HTML for theme components. Utility functions are in
 * template-functions.php, and placeholder data is in dummy-data.php.
 */
...
{
    $cached = get_transient("ballstreet_ticker_items");
    if ($cached !== false) {
        return $cached;
    }
    ...
    if ($deals_query->have_posts()) {
        ...
        $min_items = 8;
        if (count($ticker_items) < $min_items && defined("WP_DEBUG") && WP_DEBUG) {
        ...
        set_transient("ballstreet_ticker_items", $ticker_items, 5 * MINUTE_IN_SECONDS);
        return $ticker_items;
    }

    // Fallback to static ticker items (only in debug mode)
    if (defined("WP_DEBUG") && WP_DEBUG) {
        $fallback = ballstreet_get_ticker_dummy_items();
        set_transient("ballstreet_ticker_items", $fallback, 5 * MINUTE_IN_SECONDS);
        return $fallback;
    }
...
function ballstreet_render_hero_article($post = null): void
{
    ...
    $time_ago =
        human_time_diff(get_the_time("U", $post), time()) .
        " ago";
    ?>
...
function ballstreet_render_deals_grid(
    ...
    $cache_key = "ballstreet_deals_grid_{$count}_" . ($featured_only ? "featured" : "all");
    ...
    // Fill remaining slots with dummy data if needed (debug mode only)
    $remaining = $count - $rendered_count;
    if ($remaining > 0 && defined("WP_DEBUG") && WP_DEBUG) {
...
function ballstreet_render_article_rows(int $count = 5): void
{
    ...
    if ($query->have_posts()):
        ...
        endwhile;
        wp_reset_postdata();
        // Show placeholder content in debug mode, empty state in production
    else:
        if (!(defined("WP_DEBUG") && WP_DEBUG)): ?>
            <div class="articles-empty fade-in">
                <p>No articles yet. Check back soon for the latest sports business news.</p>
            </div>
        <?php else:
        $placeholders = [
            [
...
{
    $cache_key = "ballstreet_athlete_rows_{$count}";
    $cached = get_transient($cache_key);
    if ($cached !== false) {
        echo $cached;
        return;
